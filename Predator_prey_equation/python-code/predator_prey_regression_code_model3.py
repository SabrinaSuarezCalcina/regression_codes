...
# Read data files
fname_pars = '/Predator_prey_equation/File3_CSV/train_Beta_Parameter_values.csv' 
par_values = pd.read_csv(fname_pars)
fname_pars1 = '/Predator_prey_equation/File3_CSV/test_Beta_Parameter_values.csv' 
par_values1 = pd.read_csv(fname_pars1)

for j in range(3,20):
    fname_features = '/Predator_prey_equation/File3_CSV/train_TDA_Beta_features_m_%d.csv' %(j)  
    fname_features1 = '/Predator_prey_equation/File3_CSV/test_TDA_Beta_features_m_%d.csv' %(j)  
    print('file number', j) 
    ...
    np.savetxt("/Predator_prey_equation/Results-files3/R2_rmse/knn_measures_m%d.txt"%(j), final, fmt="%.14f", delimiter=",")

    final1 = np.array(matrix2)
    np.savetxt("/Predator_prey_equation/Results-files3/R2_rmse/svr_measures_m%d.txt"%(j), final1, fmt="%.14f", delimiter=",")
print('end')
