...
Cs = np.arange(5, 30, 5)
Nus = np.arange(0.2, 1.0, 0.1)
params = {'nu' : Nus, 'C': Cs}

# Read data files
fname_pars = '/Predator_prey_equation/File1_CSV/Beta_Parameter_values.csv'
par_values = pd.read_csv(fname_pars)

for j in range(10,50):
    fname_features = '/Predator_prey_equation/File1_CSV/TDA_Beta_features_m_%d.csv' %(j)   
    print('file number', j)
    ...
    for time in range(0,30):
        ...
        modelknn = regress_knn(10, X_train, X_test, y_train, y_test)
    ...
    final1 = np.array(matrix1)
    np.savetxt("/Predator_prey_equation/Results-files1/R2_rmse/knn_measures_m%d.txt"%(j), final1, fmt="%.14f", delimiter=",")

    final2 = np.array(matrix2)
    np.savetxt("/Predator_prey_equation/Results-files1/R2_rmse/svr_measures_m%d.txt"%(j), final2, fmt="%.14f", delimiter=",")
print('end')
