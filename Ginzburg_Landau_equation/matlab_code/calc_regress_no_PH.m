classdef calc_regress_no_PH
    ...
    methods (Static = true)
        function executar()                  
            ...
            for i = 1: length(beta)
            calc_regress_no_PH.form_vector(num2str(beta(i),'%.1f'))
            end
            number_samples = 2107;
        ...
        function form_vector(beta)
            entry_path = ['../Matrix_Ginzburg_Landau_Equation/Param_beta' num2str(beta)];
        ...
        function calc_File_mat(number_samples, beta)
            ...
            for n_beta = 1:length(beta)                                
                entry_path = ['../Vector/Param_beta' num2str(beta(n_beta),'%.1f')];
                ...
                for i = 1:length(file_na)  
                    ...
                    celula(ii,3) = {AA};
                    celula(ii,4) = {num2str(beta(n_beta),'%.1f')};                    
                    ii =  ii + 1;
                end                
            end
