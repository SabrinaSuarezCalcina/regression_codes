...
# Read data files
fname_pars = '/Ginzburg_Landau_equation/File5_CSV/train_Beta_Parameter_values.csv'
par_values = pd.read_csv(fname_pars)
fname_pars1 = '/Ginzburg_Landau_equation/File5_CSV/test_Beta_Parameter_values.csv'
par_values1 = pd.read_csv(fname_pars1)

for j in range(3,20):
    fname_features = '/Ginzburg_Landau_equation/File5_CSV/train_TDA_Beta_features_m_%d.csv' %(j)
    fname_features1 = '/Ginzburg_Landau_equation/File5_CSV/test_TDA_Beta_features_m_%d.csv' %(j)
    print('file number', j)
    ...
    np.savetxt("/Ginzburg_Landau_equation/Results-files5/R2_rmse/knn_measures_m%d.txt"%(j), final, fmt="%.14f", delimiter=",")

    final1 = np.array(matrix2)
    np.savetxt("/Ginzburg_Landau_equation/Results-files5/R2_rmse/svr_measures_m%d.txt"%(j), final1, fmt="%.14f", delimiter=",")
print('end')
