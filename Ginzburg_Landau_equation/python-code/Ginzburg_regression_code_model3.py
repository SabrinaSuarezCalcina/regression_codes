...
#Beta = [0.1, 1.0, 1.2, 1.4, 1.6, 1.8, 4.2]
def prep_process(modelo, X_train, y_train, X_train_n, X_test_n, y_train_n, y_test_n):
...
def regress_knn(n_neighbors, X_train, y_train, X_train_n, X_test_n, y_train_n, y_test_n): 
    knn = KNeighborsRegressor(n_neighbors, weights='distance')  # Simple method for creating regressions that aren’t linear    
    regr_knn = prep_process(knn, X_train, y_train, X_train_n, X_test_n, y_train_n, y_test_n)
    return(regr_knn[0], regr_knn[1]) 
   
def regress_NuSVR(X_train, y_train, X_train_n, X_test_n, y_train_n, y_test_n, C1, nu1): 
    nusvr = NuSVR(nu = nu1, C = C1, kernel= 'rbf', gamma=0.0001, tol = 0.001)
    regr_nusvr = prep_process(nusvr, X_train, y_train, X_train_n, X_test_n, y_train_n, y_test_n)
...
# Read data files
fname_pars = '/Ginzburg_Landau_equation/File6_CSV/train_Beta_Parameter_values.csv'
par_values = pd.read_csv(fname_pars)
fname_pars1 = '/Ginzburg_Landau_equation/File6_CSV/test_Beta_Parameter_values.csv'
par_values1 = pd.read_csv(fname_pars1)

for j in range(3,20):
    fname_features = '/Ginzburg_Landau_equation/File6_CSV/train_TDA_Beta_features_m_%d.csv' %(j)    
    fname_features1 = '/Ginzburg_Landau_equation/File6_CSV/test_TDA_Beta_features_m_%d.csv' %(j)    
    print('file number', j)
    ...
    for time in range(0,30):
        ...
        modelknn = regress_knn(7, X_train, y_train, X_train_n, X_test_n, y_train_n, y_test_n)
        ...
        modelsvr = regress_NuSVR(X_train, y_train, X_train_n, X_test_n, y_train_n, y_test_n, par_C, par_nu)   
    ...
    np.savetxt("/Ginzburg_Landau_equation/Results-files6/R2_rmse/knn_measures_m%d.txt"%(j), final, fmt="%.14f", delimiter=",")

    final1 = np.array(matrix2)
    np.savetxt("/Ginzburg_Landau_equation/Results-files6/R2_rmse/svr_measures_m%d.txt"%(j), final1, fmt="%.14f", delimiter=",")
print('end')
